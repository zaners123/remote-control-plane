...
public class App {
    ...
    private final static int GPIO_PROP = 1;
    private final static int GPIO_RUDDER = -1;
    private final static int GPIO_LEFT_AILERON = 2;
    private final static int GPIO_RIGHT_AILERON = -1;
    private final static int GPIO_LEFT_ELEVATOR = -1;
    private final static int GPIO_RIGHT_ELEVATOR = -1;
    ...
    private void sensorData() {
        try {
            ...
            for (int i = 0; i < 1; i++) {
                ...
                v = sensor.getMagnetometer();
                System.out.println("Magnetometer: X="+v.x+"Y="+v.y+"Z="+v.z);

                v = sensor.getAngularVelocity();
    ...
    class BNO055 {
        ...
        Vector getGravity() throws IOException, SensorException {
            Vector v = readVector(BNO055_GRAVITY_DATA_X_LSB_ADDR, 3);
            //scale linear acceleration; see datasheet
            v.scale(1f/100);
            return v;
        }
